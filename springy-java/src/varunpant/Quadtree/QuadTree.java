 ...
 * can not re-balance itself to that degree.
 */
public class QuadTree {
    ...
    public Point[] getKeys() {
        ...
        this.traverseLeafNode(this.root_, new Func() {
            @Override
    ...
    public Point[] searchWithin(final double xmin, final double ymin, final double xmax, final double ymax) {
        ...
        this.navigate(this.root_, new Func() {
            ...
            public void call(QuadTree quadTree, Node node) {
                Point pt = node.getPoint();
                if (pt.isBetween(xmin, ymin, xmax, ymax)) {                
                    arr.add(pt);
                }
            }
        }, xmin, ymin, xmax, ymax);
     ...
     * @private
     */
    public void traverseLeafNode(Node node, Func func) {
        switch (node.getNodeType()) {
            case LEAF:
                func.call(this, node);
                break;
            ...
            case POINTER:
                this.traverseLeafNode(node.getNe(), func);
                this.traverseLeafNode(node.getSe(), func);
                this.traverseLeafNode(node.getSw(), func);
                this.traverseLeafNode(node.getNw(), func);
                break;
        }
    }

    public void traverseNode(Node node, Func func) {
    	List<Node> nodeQueue = new LinkedList<Node>();
    	
    	nodeQueue.add(this.getRootNode());
    	while (nodeQueue.size() > 0) {
    		Node qnode = nodeQueue.remove(0);
            boolean isInternal = !qnode.isLeaftNode();

            //
            func.call(this, qnode);

//            // do not append children nodes if result === false
//            if (result === false) {
//                continue;
//            }

            if (isInternal) {
//                nodeQueue.addAll(qnode.);
//                (nodeQueue, qnode.nodes);
            }
        ...
        switch (node.getNodeType()) {
            ...
            case POINTER:
                this.traverseNode(node.getNe(), func);
                this.traverseNode(node.getSe(), func);
                this.traverseNode(node.getSw(), func);
                this.traverseNode(node.getNw(), func);
                break;
        }
    }
